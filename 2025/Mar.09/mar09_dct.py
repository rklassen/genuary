...
channels = img.split()
os.makedirs("./2025/Mar.09/debug/", exist_ok=True)
...
for index in range(3):
    ...
    ch.convert("L").save(f"./2025/Mar.09/debug/source_{name}.png")
    this_dct = dctn(numpy.array(ch, dtype=float))
    dct_data.append(this_dct)
    Image.fromarray(this_dct.astype(numpy.uint8)).save(f"./2025/Mar.09/debug/dct_{name}.png")
    this_dct_normalized = (this_dct - this_dct.min()) / (this_dct.max() - this_dct.min()) * 255

for radius in range(r_max - 1, -1, -1):
    this_frame_ch = [ ]
    ...
    merged.save(f"./2025/Mar.09/frames/frame {str(radius).zfill(3)}.png")
